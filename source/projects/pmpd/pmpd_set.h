void pmpd_setK(t_pmpd *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if ( atom_gettype(argv) == A_SYM )
    {
        ...
        {
            t_symbol *id_ = atom_getsym(argv);
            double val = atom_getNumber(argv+1);
    ...
    else if (atom_isNumber(argv))
    {
        if (atom_isNumber(argv+1))
        {
            tmp = atom_getNumber(argv);
            tmp = max(0, min( x->nb_link-1, tmp));
            x->link[tmp].K = atom_getNumber(argv+1);
        }
    }
}
//
...
//            {
//                x->link[i].K = atom_getNumber(argv+1);
//            }
//        }
//    }
//}
...
//        x->link[tmp].D = atom_getfloatarg(1, argc, argv);
        x->link[tmp].D = atom_getNumber(argv+1);
    }
    if ( ( atom_gettype(argv) == A_SYM ) && atom_isNumber(argv+1) )
    {
        for (i=0; i< x->nb_link; i++)
        {
            if ( atom_getsym(argv) == x->link[i].Id)
            {
                x->link[i].D = atom_getNumber(argv+1);
            }
        }
    }
}
...
void pmpd_setD2(t_pmpd *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if ( atom_isNumber(argv) && atom_isNumber(argv+1) )
    {
        ...
        x->mass[tmp].D2 = atom_getNumber(argv+1);
    }
    if ( ( atom_gettype(argv) == A_SYM ) && atom_isNumber(argv+1) )
    {
...
{
    int tmp, i;
    double K_l = atom_getfloatarg(1, argc, argv);
    if (K_l <=  0) K_l = 1;
    if ( ( argv[0].a_type == A_FLOAT ) && ( argv[1].a_type == A_FLOAT ) )
    {
        tmp = atom_getfloatarg(0, argc, argv);
...
{
    int tmp, i;
    double D_l = atom_getfloatarg(1, argc, argv);
    if (D_l <=  0) D_l = 1;
    if ( ( argv[0].a_type == A_FLOAT ) && ( argv[1].a_type == A_FLOAT ) )
    {
        tmp = atom_getfloatarg(0, argc, argv);
...
void pmpd_setLinkId(t_pmpd *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if ( ( argv[0].a_type == A_FLOAT ) && ( argv[1].a_type == A_SYM ) )
    {
        tmp = atom_getfloatarg(0, argc, argv);
        tmp = max(0, min( x->nb_link-1, tmp));
...
void pmpd_setEnd(t_pmpd *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    else if ( (argc == 3) && ( argv[0].a_type == A_SYM ) && ( argv[1].a_type == A_FLOAT ) && ( argv[2].a_type == A_FLOAT ) )
    {
        for (i=0; i< x->nb_link; i++)
        {
            if ( atom_getsymbolarg(0,argc,argv) == x->link[i].Id )
