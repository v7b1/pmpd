int test_1d_mass(int i, t_pmpd *x, int argc, t_atom *argv)
{
	...
	while (j < argc) 
	{
		...
		else
		{
			...
			{
				object_post((t_object*)x, "Option \"%s\" not reconized, stoping test",atom_getsymbolarg(j,argc,argv)->s_name);
				return(-1);
			}
		}
	}
	...
	while (j < argc) 
	{
		...
		else
		{
			...
			else if ( atom_getsymbolarg(j,argc,argv) == gensym("connectedTo") )
			{
				...
				{
					object_post((t_object*)x, "bad argument for connectedTo");
					j+=1;
				}
			}
			else
...
//            object_error((t_object*)x, "%s: no such array", tab_name->s_name);
//        else if (!garray_getfloatwords(a, &vecsize, &vec))
//            object_error((t_object*)x, "%s: bad template for tabwrite", tab_name->s_name);
//        else
//        {
