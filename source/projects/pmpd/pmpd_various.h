...
{
    int i;
    object_post((t_object*)x, "list of mass");
    object_post((t_object*)x, "number, Id, mobile, mass, Damping, position, speed, forces");
    for(i=0; i < x->nb_mass; i++)
    {
        object_post((t_object*)x, "masse %i: %s, %d, %f, %f, %f, %f, %f",i, x->mass[i].Id->s_name, x->mass[i].mobile, 1/x->mass[i].invM, x->mass[i].D2, x->mass[i].posX, x->mass[i].speedX, x->mass[i].forceX );
    }

    object_post((t_object*)x, "list of link");
    object_post((t_object*)x, "number, Id, mass1, mass2, K, D, Pow, L, Lmin, Lmax");
    for(i=0; i < x->nb_link; i++)
    {
        switch(x->link[i].lType)
        {
        case 0 :
            object_post((t_object*)x, "link %i: %s, %i, %i, %f, %f, %f, %f, %f, %f", i, x->link[i].Id->s_name, x->link[i].mass1->num, x->link[i].mass2->num, x->link[i].K, x->link[i].D, x->link[i].Pow, x->link[i].L, x->link[i].Lmin, x->link[i].Lmax);
            break;
        case 2 :
            object_post((t_object*)x, "tabLink %i: %s, %i, %i, %f, %f, %s, %f, %s, %f", i, x->link[i].Id->s_name, x->link[i].mass1->num, x->link[i].mass2->num, x->link[i].K, x->link[i].D, x->link[i].arrayK->s_name, x->link[i].K_L, x->link[i].arrayD->s_name, x->link[i].D_L);
            break;
        }
    }
}
...
void pmpd_minX(t_pmpd *x, double min)
{
    x->minX = min;
}

void pmpd_maxX(t_pmpd *x, double max)
{
    x->maxX = max;
}
...
void pmpd_grabMass(t_pmpd *x, double posX, double grab)
{
    double dist, tmp;
    t_int i;
...
void pmpd_massDistances(t_pmpd *x, t_symbol *s, int argc, t_atom *argv)
{
	...
	else if ( (argc == 2) && (argv[0].a_type == A_FLOAT) && (argv[1].a_type == A_SYM) )
	{
		for (i=0; i < x->nb_mass; i++)
		{
			if ( atom_getsymbolarg(1,argc,argv) == x->mass[i].Id)
...
//                }
//            }
//            else if (( argc > 1 ) && ( argv[1].a_type == A_SYM ))
//            {
//                i=0;
//                j=0;
//
